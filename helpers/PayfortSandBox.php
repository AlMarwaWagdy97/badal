...
class PayfortIntegration
{
     ...
     */
    public $SHAResponsePhrase = 'dxkvuigyigjk';
    ...
    public function getRedirectionData($paymentMethod)
    {
        ...
        $postData = array(
            ...
            'customer_email' =>  $this->customerEmail ?? 'almarwa.wagdy@gmail.com',
        ...
        } elseif ($paymentMethod == 'STCPAY') {
            $postData['digital_wallet'] = 'STCPAY';
        }
    ...
    {
        $merchantReference = $this->generateMerchantReference();
        $returnUrl = $this->getUrl('payfortrespond.php?r=merchantPageReturn');
        if (isset($_GET['3ds']) && $_GET['3ds'] == 'no') {
            $returnUrl = $this->getUrl('payfortrespond.php?r=merchantPageReturn&3ds=no');
        }
        $iframeParams = array(
            'merchant_identifier' => $this->merchantIdentifier,
            'access_code' => $this->accessCode,
