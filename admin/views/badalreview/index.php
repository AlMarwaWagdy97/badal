        ...
        <div class="col-md-12 col-sm-12 col-xs-12">
            <form action="" method="post">
                <div class="row">
                    ...
                    <div class="col-xs-3 form-group"><span class="title"> رقم العمليه :</span><input type="search" class="form-control" placeholder="بحث بالرقم العمليه" name="search[badal_id]" value=""></div>
                    <div class="col-xs-3 form-group"><span class="title">بحث بالنسبة :</span>
                        <select class="form-control" name="search[rate]" style="width: 100%;">
                            <option value=""></option>
                            <?php foreach ($data['reviews'] as $key => $review) {
                                echo '<option value="' . $key . '" >' . $review . '</option>';
                            } ?>
                        </select>
                    </div>
                        ...
                        <tbody>
                            <?php foreach ($data['badalreviews'] as $badalreview) : ?>
                                <tr class="even pointer">
                                    ...
                                    <td class="form-group">
                                        <a href="<?php echo ADMINURL . '/badalreviews/show/' . $badalreview->review_id; ?>" class="btn btn-xs btn-primary" data-placement="top" data-toggle="tooltip" data-original-title="تعديل" "><i class="fa fa-edit"></i></a>
                                        <a href="<?php echo ADMINURL . '/badalreviews/delete/' . $badalreview->review_id; ?>" class="btn btn-xs btn-danger" data-placement="top" data-toggle="tooltip" data-original-title="حذف" onclick="return confirm('Are you sure?') ? true : false"><i class="fa fa-trash-o"></i></a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                            ...
                            <tr class="tab-selected">
                                ...
                                <th class="column-title" colspan="3"> عرض
                                    <select name="perpage" onchange="if (this.value)
                                                window.location.href = '<?php echo ADMINURL . '/badalreviews/index/' . $data['current']; ?>' + '/' + this.value">
