                ...
                <div class="accordion">
                        ...
                        <div id="collapseOne" class="collapse card-body" aria-labelledby="headingOne">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="kafara" id="kafara">
                                    <option value="web" <?php echo ('web' == $data['kafara']) ? " selected " : ''; ?>>في الويب فقط</option>
                                    <!-- <option value="both" <?php echo ('both' == $data['kafara']) ? " selected " : ''; ?>> في الويب والتطبيق</option> -->
                                    <option value="app" <?php echo ('app' == $data['kafara']) ? " selected " : ''; ?>>في التطبيق فقط</option>
                                </select>
                            </div>
                            <div class="form-group">
                    ...
                    <div class="card">
                        ...
                        <div id="collapseThree" class="collapse card-body <?php echo (empty($data['background_image_error'])) ?: 'in'; ?>" aria-labelledby="headingThree">
                            ...
                            <div class="form-group <?php echo (empty($data['ads_banner_error'])) ?: 'has-error'; ?>">
                                <label class="control-label">صورة البنر الفاصل في الصفحة الرئيسية : </label>
                                <div class="has-feedback input-group">
                                    <span class="input-group-btn">
                                        <span class="btn btn-dark" onclick="$(this).parent().find('input[type=file]').click();">اختار الملف</span>
                                        <input name="ads_banner" value="<?php echo ($data['ads_banner']); ?>" onchange="$(this).parent().parent().find('.form-control').html($(this).val().split(/[\\|/]/).pop());" style="display: none;" type="file">
                                    </span>
                                    <span class="form-control"><small><?php echo empty($data['ads_banner']) ? 'قم بأختيار صورة مناسبة' : $data['ads_banner']; ?></small></span>
                                </div>
                                <div class="help-block"><?php echo $data['ads_banner_error']; ?></div>
                            </div>
                            <div class="form-group">
                                <label>الرابط الخاص بالبنر : </label>
                                <input type="text" class="form-control" name="ads_url" value="<?php echo $data['ads_url']; ?>">
                            </div>


                            <div class="form-group <?php echo (empty($data['ads2_banner_error'])) ?: 'has-error'; ?>">
                                <label class="control-label">صورة البنر2 الفاصل في الصفحة الرئيسية : </label>
                                <div class="has-feedback input-group">
                                    <span class="input-group-btn">
                                        <span class="btn btn-dark" onclick="$(this).parent().find('input[type=file]').click();">اختار الملف</span>
                                        <input name="ads2_banner" value="<?php echo ($data['ads2_banner']); ?>" onchange="$(this).parent().parent().find('.form-control').html($(this).val().split(/[\\|/]/).pop());" style="display: none;" type="file">
                                    </span>
                                    <span class="form-control"><small><?php echo empty($data['ads2_banner']) ? 'قم بأختيار صورة مناسبة' : $data['ads2_banner']; ?></small></span>
                                </div>
                                <div class="help-block"><?php echo $data['ads2_banner_error']; ?></div>
                            </div>
                            <div class="form-group">
                                <label>2الرابط الخاص بالبنر : </label>
                                <input type="text" class="form-control" name="ads2_url" value="<?php echo $data['ads2_url']; ?>">
                            </div>


                            <div class="form-group">
                                <label>لون خلفية القسم بالصفحة الرئيسية : </label>
                                <input type="text" class="colorpicker form-control" name="section_bg" value="<?php echo $data['section_bg']; ?>" data-wcp-format="rgba">
                            </div>
                        </div>
                    </div>
