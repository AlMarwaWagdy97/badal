...
class Managers extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->menu = $this->managerModel->getMenu();
        $this->settings = $this->managerModel->getSettings();
    }
    /**
     * redirect with alias to load shorter manager URL
     *
     * @return void
     */
    public function index()
    {
        redirect('managers/login', true);
    }
    /**
     * login manager admin by id
     *
     * @param  int $id
     *
     * @return view
    ...
    public function login()
    {
        ...
        if (isset($_SESSION['managerlogin'])) redirect('managers/orders', true); // check if user already logged in
        //check for post
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $data = [
                ...
                'email_error' => '',
                'password_error' => '',
                'pageTitle' => 'تسجيل دخول مدير المتاجر  : ' . SITENAME,
                'settings' => $this->settings,
                'menu' => $this->menu,
            ];

            if (empty($data['email'])) { //validate user
                $data['email_error'] = '<div class="alert alert-danger">لا يمكن ترك حقل المستخدم خاليا</div>';
            } elseif (!$user = $this->managerModel->findUser($data['email'])) {
                $data['email_error'] = '<div class="alert alert-danger">هذ المستخدم ليس مسجل لدينا</div>';
            }

            if (empty($data['password'])) { //validate password
                $data['password_error'] = '<div class="alert alert-danger">لا يمكن ترك حقل كلمة المرور خاليا</div>';
            }
            if (empty($data['email_error']) && empty($data['password_error'])) {
                ...
                if (password_verify($data['password'], $user->password)) { //check and login user
                    ...
                    redirect('managers/orders', true);
                } else {
                    $data['password_error'] = '<div class="alert alert-danger">كلمة المرور غير صحيحة</div>';
                }
            }
        } else {
            $data = [
    ...
    public function logout()
    {
        ...
        flashRedirect('managers/login', 'msg', 'تم تسجيل الخروج بنجاح  ', 'alert alert-info');
    }
    /**
     * view manager orders
     *
    ...
    public function orders($current = '', $perpage = 50)
    {
        ...
        if (!isset($_SESSION['managerlogin'])) redirect('managers/login', true); // check if user already logged in
        // get orders
        $cond = 'WHERE ord.status <> 2 AND donors.donor_id = ord.donor_id AND ord.payment_method_id = payment_methods.payment_id AND stores.store_id = ord.store_id AND  ord.store_id  in (' . $stores . ') ';
        $bind = [];
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (isset($_POST['search']['submit'])) {
                ...
                if (!empty($_POST['search']['store_id'])) {
                    $cond .= ' AND ord.store_id = ' . $_POST['search']['store_id'];
                }
                // date search
                ...
                //manager status and payment method for saving search attribute 
                ...
                if (isset($_POST['search']['manager_id'])) $_SESSION['search']['manager_id'] = $_POST['search']['manager_id'];
        ...
        $recordsCount = $this->managerModel->allOrdersCount(", donors, payment_methods, stores " . $cond, $bind);
        ...
        ($manager = $this->managerModel->getManagerById($_SESSION['managerlogin']->manager_id)) ?: flashRedirect('index', 'msg', '  حدث خطأ ما ');
        $_SESSION['manager'] = $_SESSION['managerlogin'];
        $_SESSION['manager']->manager_id = $_SESSION['managerlogin']->manager_id;
        //get all records for current order
        $orders = $this->managerModel->getOrders($cond, $bind, $limit, $bindLimit);
        $stores = $this->managerModel->getStores();
        $data = [
        ...
        $this->view('managers/orders', $data);
    }
    ...
    public function password()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            // sanitize POST array
            $_POST = filter_input_array(INPUT_POST, FILTER_SANITIZE_FULL_SPECIAL_CHARS);
            echo $this->managerModel->updatePassword($_POST['manager_id'], $_POST['password']);
            redirect('managers/logout', true);
        } else {
