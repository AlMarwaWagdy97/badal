...
<?php flash('msg'); ?>
<div class="container-md px-0 undermenu text-center">
    <div class="row no-gutters border rounded">
        <div class="col-md-4">
            <img src="<?php echo (empty($data['category']->image)) ? MEDIAURL . '/default.jpg' : MEDIAURL . '/' . $data['category']->image; ?>" class="rounded-circle p-4 img-fluid" alt="<?php echo $data['category']->name; ?>">
        </div>
        <div class="col-md-8">
            <div class="card-body">
                <h1 class="card-title text-primary h3"><?php echo $data['category']->name; ?></h1>
    ...
    <div id="categories" class="d-flex justify-content-center">
        <?php if (count($data['subcategories']) > 0) : ?>
            <?php foreach ($data['subcategories'] as $category) { ?>
                <div class="project-category col-lg-3 col-md-4">
                    <div class="category-container my-3">
                        <img class="rounded-circle img-fluid p-4" src="<?php echo (empty($category->image)) ? MEDIAURL . '/default.jpg' : MEDIAURL . '/' . $category->image; ?>" alt="<?php echo $category->name; ?>">
                        ...
                        <a class="btn btn-outline-secondary btn-sm mb-3 category-btn" href="<?php echo URLROOT . '/projectCategories/show/' . $category->category_id . '-' . $category->name; ?>">التفاصيل</a>
                    </div>
                </div>
...
<section id="projects">
    <div class="container">
            ...
            <div class="row my-4 ">
                <div class="col">
                    <h3 class="text-center">
                        ...
                        <?php echo $data['settings']['site']->project_text; ?>
                    </h3>
                </div>
            </div>
                    ...
                    <div class="col-12 col-xl-4 col-md-6 my-2 px-1">
                        <div class="project bg-light">
                            <div class="project-img">
                                ...
                                <ul class="project-social p-0 ml-1 nav flex-column" data-toggle="collapse" data-target="#toggelShare<?php echo $project->project_id ?>">
                                    ...
                                    <span class="collapse toggelShare" id="toggelShare<?php echo $project->project_id ?>">
                                        <?php   foreach(sortSetting($data['settings']['social']->sort) as $key => $share){ ?>
                                            <?php  if($key == "whatsapp") { ?>
                                                <a target="blank" class="nav-link" href="https://wa.me/?text=<?php echo URLROOT . '/projects/show/' . $project->project_id; ?> " style="color: #<?= @$data['settings']['social']->color->products ?> ">
                                                    <i class="icofont-whatsapp "></i>
                                                </a>
                                            <?php } if($key == "facebook") { ?>
                                                <a target="blank" class="nav-link" href="https://www.facebook.com/sharer/sharer.php?u=<?php echo URLROOT . '/projects/show/' . $project->project_id; ?>" style="color: #<?= @$data['settings']['social']->color->products ?> ">
                                                    <i class="icofont-facebook "></i>
                                                </a>
                                            <?php } if($key == "twitter") { ?>
                                                <a target="blank" class="nav-link" href="https://twitter.com/intent/tweet?url=<?php echo URLROOT . '/projects/show/' . $project->project_id; ?>&text=<?php echo $project->name; ?>"" style="color: #<?= @$data['settings']['social']->color->products ?> ">
                                                    <i class="icofont-twitter"></i>
                                                </a>
                                            <?php } if($key == "email") { ?>
                                                <a target="blank" class="nav-link" href="mailto:?&subject=&cc=&bcc=&body=<?php echo URLROOT . '/projects/show/' . $project->project_id; ?>" style="color: #<?= @$data['settings']['social']->color->products ?> ">
                                ...
                                <div class="target p-3">
                                    <small class=" pt-1 float-right"> <span> المستهدف:
                                            ...
                                            <?php if (empty($project->target_unit)) {
                                                 echo ' <img src="' . MEDIAURL . '/rayal.png" width="10px">';
                                            } else {
        ...
        <?php endif;  ?>
        <div class="row ">
            <?php echo ($data['category']->back_home) ? '<div class="col-md-6 mx-auto my-4"><a class="w-100 btn btn-lg btn-secondary icofont-home" href="' . URLROOT . '"> العودة الي الرئيسية</a></div>' : ''; ?>
        </div>
</section>
<!-- alertModal -->
<div class="modal fade text-left" id="alertModal">
