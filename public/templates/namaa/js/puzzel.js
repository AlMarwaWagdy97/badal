...
const frame = new Frame("fit", width, hight, light, light, image, url);
frame.on("ready", () => {
  ...
  var song = frame.asset("hierarchies.mp3").play();
  // CODE HERE
  ...
  const pic = asset(image).clone().center().alp(0.3).vis(false); // checkbox later to turn on
  ...
  const pics = chop(asset(image), numX, numY, false, extra);
  ...
  function makePiece() {
    ...
    let nextX = rand() > 0.5 ? 1 : -1; // randomize the next 1 or -1 for out or in horizontally
  ...
  function test(piece) {
    ...
    if (piece.rotation % 360 == 0 && box.hitTestReg(piece)) {
      ...
      placed++;
      if (placed == num) { //puzzel completed
        $("body").append(modal.show(1000));// append the form modal
        $('.timing').val(upto) // get the complete in time
        timeout = 100000;// set time out for very long time
        ...
        timeout(2, function () {
          ...
          new Button({
            ...
            color: white,
            corner: [60, 0, 60, 0],
            backgroundColor: blue.darken(0.3),
            ...
            .tap(() => {
              zgo("https://namaa.sa/puzzle", "_blank");
            });
        });
      } else
        stats.text = `Placed ${placed} piece${
          placed == 1 ? "" : "s"
        } of ${num}`;
    } else stage.update();
  }
  ...
  const hintCheck = new CheckBox(30, "hint")
    .alp(0.8)
  ...
  // const picCheck = new CheckBox(30, "picture")
  //   .alp(0.8)
  //   .pos(150, 50, LEFT, BOTTOM)
  //   .wire({ target: pic, prop: "visible", input: "checked" });
  // picCheck.checked = true;

  // new Label("ZIM JigSaw - tap or shift tap to rotate").pos(0,30,CENTER);
  // new Label("ZIM JigSaw - tap or shift tap to rotate").pos(0,50,CENTER);
